...
export function usePlayerEngagementReport(
  ...
  options?: Omit<UseQueryOptions<PlayerEngagementReport[]>, 'queryKey' | 'queryFn'>
) {
  return useQuery({
...
export function usePlayerEngagementStats(filters?: PlayerEngagementFilters) {
  ...
  const stats = {
    ...
    avgPlaytime: data.reduce((sum, p) => sum + p.total_playtime_hours, 0) / data.length,
    avgRetention: data.reduce((sum, p) => sum + p.retention_score, 0) / data.length,
    avgAchievements: data.reduce((sum, p) => sum + p.achievements_unlocked, 0) / data.length,
...
export function useActivePlayers(minRetentionScore: number = 70) {
  ...
  const activePlayers = data?.filter(p => p.retention_score >= minRetentionScore);
...
export function useAtRiskPlayers(maxRetentionScore: number = 30) {
  ...
  const atRiskPlayers = data?.filter(p => p.retention_score <= maxRetentionScore);
