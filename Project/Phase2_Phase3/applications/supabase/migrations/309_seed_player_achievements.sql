...
CREATE OR REPLACE FUNCTION populate_player_achievements() RETURNS void AS $$
...
SELECT id
FROM "public"."players" LOOP achievement_count := (random() * 7 + 3)::int;
FOR i IN 1..achievement_count LOOP
SELECT id INTO achievement_record
...
END LOOP;
END LOOP;
RAISE NOTICE 'Populated % player achievement unlocks',
total_unlocks;
END;
$$ LANGUAGE plpgsql;