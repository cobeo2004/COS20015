...
CREATE OR REPLACE FUNCTION populate_sessions() RETURNS void AS $$
...
SELECT id
FROM "public"."players" LOOP session_count := (random() * 5 + 3)::int;
FOR i IN 1..session_count LOOP
SELECT id INTO game_record
FROM "public"."games"
ORDER BY random()
LIMIT 1;
...
INSERT INTO "public"."sessions" (player_id, game_id, start_time, end_time, score)
VALUES (
        player_record.id,
        game_record.id,
        start_time,
...
END LOOP;
END LOOP;
RAISE NOTICE 'Populated % sessions',
total_sessions;
END;
$$ LANGUAGE plpgsql;