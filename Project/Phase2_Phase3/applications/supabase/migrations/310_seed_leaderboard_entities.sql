...
CREATE OR REPLACE FUNCTION populate_leaderboard_entities() RETURNS void AS $$
DECLARE leaderboard_record RECORD;
...
SELECT id
FROM "public"."leaderboards" LOOP -- Each leaderboard has 10-20 players ranked
...
FROM "public"."players"
ORDER BY random()
LIMIT entry_count LOOP
INSERT INTO "public"."leaderboard_entities" (
        leaderboard_id,
        player_id,
        rank,
        score,
...
END LOOP;
END LOOP;
RAISE NOTICE 'Populated % leaderboard entries',
total_entries;
END;
$$ LANGUAGE plpgsql;